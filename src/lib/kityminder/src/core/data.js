define(function (require, exports, module) {
    ...
    kity.extendClass(Minder, {
         ...
         * @param parentNode
         */
        appendNodes: function (json, parentNode) {
            var that = this;
            var selectNode;
            json.forEach(function (item) {
                selectNode = that.createNode(item.data, parentNode);
                ...
                if (item.children && item.children.length > 0) {
                    that.appendNodes(item.children, selectNode);
                }
            });
        },
         ...
         * @returns {Array}
         */
        exportSnapshootDataList: function () {
            var resultList = [],
                obj;
            ...
            function getData(nodes) {
                nodes.forEach(function (item) {
                    ...
                    for (var key in item.data) {
                        if (key.indexOf('layout') > -1 || key.indexOf('ExpandState') > -1) {
            ...
            obj = {id: this._root.data.id};
            resultList.push(obj);
            getData(this._root.children);
            return resultList;
         ...
         * @param snapShootList
         */
        importSnapshootDataList: function (snapShootList) {
            var self = this, node;
            if (!snapShootList || !snapShootList.length)return;
            snapShootList.forEach(function (item) {
                node = self.getNodeById(item.id);
                for (var key in item) {
                    node.data[key] = item[key];
                }
            });
            this._root.renderTree();
        }
