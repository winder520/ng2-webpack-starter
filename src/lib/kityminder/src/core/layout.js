    ...
    kity.extendClass(Minder, {
        ...
        layout: function () {
            ...
            function layoutNode(node, round) {
                ...
                // 剪枝：收起的节点无需计算
                if (hasChildVisible && node.isVisible(true) && node.isExpanded()) {
                    node.children.forEach(function (child) {
                        layoutNode(child, round);
                    });
                }
                ...
                // });
                layout.doLayout(node, node.getChildren(), round, hasChildVisible);
                // setTimeout((function (node) {
                //     if (node._expand)
                //         node._expand.setVisible(hasChildVisible);
                // }).bind(null, node), 500);
