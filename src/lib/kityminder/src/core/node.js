define(function (require, exports, module) {
    ...
    var MinderNode = kity.createClass('MinderNode', {
        ...
        constructor: function (textOrData) {
            ...
            if (utils.isString(textOrData)) {
                ...
                textOrData.connect = textOrData.connect || {};
                if ('advanced,class,enum,anonymity'.indexOf(textOrData.category) > -1) {
                    textOrData.connect.type = textOrData.connect.type || 'subClassOf';
                } else if ('property' === textOrData.category) {
                    textOrData.connect.type = textOrData.connect.type || 'subPropertyOf';
                }
        ...
        isReadOnly: function () {
            return this.data.isReference || this.data.from === 'import' || this.data.category === 'text';
        },

        isVisible: function (visible) {
            var rootNode = this.getRoot();
            rootNode.data.graphType = rootNode.data.graphType || 'class-class';
            rootNode.data.config = rootNode.data.config || {};
            switch (true) {
                case rootNode.data.graphType === 'class-class':
                    if (this.data.category === 'property') {
                        visible = false;
                    } else if (this.parent && this.parent.data.category === 'propertyRestriction') {
                        visible = false;
                    } else if (this.parent && this.parent.data.category === 'property') {
                        visible = false;
                    }
                    if (rootNode.data.config.isFilterPropertyRestriction && 'propertyRestriction' === this.data.category) {
                        visible = false;
                    }
                    break;
                case rootNode.data.graphType === 'class-property':
                    if (this.data.category === 'property') {
                        visible = false;
                    } else if ('advanced,class,enum,anonymity'.indexOf(this.data.category) > -1 && this.parent && this.parent.data.category === 'property') {
                        visible = false;
                    }
                    if ('advanced,class,enum,anonymity'.indexOf(this.data.category) > -1 && this.parent && 'propertyRestriction' !== this.parent.data.category) {
                        visible = false;
                    }
                    break;
                case rootNode.data.graphType === 'property':
                    if ('advanced,class,enum,anonymity'.indexOf(this.data.category) > -1 && this.parent && this.parent.data.category !== 'property') {
                        visible = false;
                    } else if (this.parent && 'advanced,class,enum,anonymity'.indexOf(this.parent.data.category) > -1) {
                        visible = false;
                    } else if (this.parent && 'propertyRestriction' === this.parent.data.category) {
                        visible = false;
                    }
                    break;
                default:
            ...
            if (!this.parent || (this.parent && this.parent.isRoot()))
                return visible;
            else
                return this.parent.isVisible(visible);
        }
    });
    ...
    kity.extendClass(Minder, {
        ...
        getNodesByIri: function (iri, filter) {
            var nodes = this.getAllNode();
            var result = [];
            ...
            nodes.forEach(function (node) {
                if (node.getData('iri') === iri && filter(node)) {
                    result.push(node);
                }
            });
            return result;
        ...
        removeSelectNode: function (node) {
            if (node.parent) {
                ...
                node.attached = false;
                node.parent.removeChild(node);
                rc.removeShape(node.getRenderContainer());
         ...
         * @returns {*|string}
         */
        getLayoutGraphType: function () {
            return this.getRoot().data.graphType || 'class-class';
        }
