define(function (require, exports, module) {
    ...
    Module.register('Expand', function () {
        ...
        kity.extendClass(MinderNode, {
             ...
             * 判断节点当前的状态是否为展开
             */
            isExpandedByNode: function () {
                return true;
                /*var expanded = (!this.data.connect || !this.data.connect.type);
                 if (!expanded && this.parent)
                 expanded = this.parent.getData(this.data.connect.type + 'ExpandState') !== STATE_COLLAPSE;
                 return expanded && (this.isRoot() || this.parent.isExpandedByNode());*/
            },
         ...
         * @description 展开当前选中的节点，保证其可见
         * @param {object} obj 隐藏或展开哪一种类型节点
         * @state
         *   0: 当前有选中的节点
         *  -1: 当前没有选中的节点
         */
        var ExpandCollapseCommand = kity.createClass('ExpandCollapseCommand', {
            base: Command,

            execute: function (km, params) {
                var node = km.getSelectedNode();
                if (!node || !params) return;

                node.data[params.type] = node.data[params.type] !== STATE_EXPAND ? STATE_EXPAND : STATE_COLLAPSE;
        ...
        var ExpanderRenderer = kity.createClass('ExpanderRenderer', {
            ...
            update: function (expander, node, box) {
                ...
                for (var i = 0, len = node.children.length; i < len; i++) {
                    if (node.children[i].isVisible(true)) {
        ...
        return {
            commands: {
                ...
                'expandcollapsenodetype': ExpandCollapseCommand
            },
            events: {
                ...
                'beforerender': function (e) {
                    ...
                    if (visible)
                        visible = !node.parent || node.isExpandedByNode();
