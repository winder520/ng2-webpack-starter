    ...
    function InputRuntime() {
        ...
        function setupFsm() {
            ...
            minder.on('dblclick', function (e) {
                ...
                minder._executeEventImmediately(e.type, {type: 'node', element: selectNode, event: e});
                // if (minder.getStatus() !== 'readonly' && selectNode && !selectNode.isRoot() && !selectNode.isReadOnly() && selectNode.data.category !== 'anonymity' && 'class,enum,advanced,individual'.indexOf(selectNode.data.category)===-1&& selectNode.data.category !== 'propertyRestriction') {
                //     editText();
                // }
            });
        }
